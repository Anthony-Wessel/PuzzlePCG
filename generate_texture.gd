...
func generate_mask(triangle_count : int, points : Array[Vector2]):
	...
	var uniform_set := rd.uniform_set_create([output_tex_uniform, size_uniform, triangles_uniform], shader, 0)
	...
	#rd.free_rid(size_uniform_set)
	#rd.free_rid(size_buffer)
	
	#rd.free_rid(triangles_uniform_set)
	#rd.free_rid(triangles_buffer)
